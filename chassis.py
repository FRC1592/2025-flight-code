...
class Chassis:
    ...
    def drive_field_relative(self,
                             ...
                             theta : units.radians_per_second):
    ...
    def _set_pod(self, pod : int, command : SwerveModuleState):
        ...
        self._drives[pod].set(phoenix6.controls.DutyCycleOut().ControlMode.PercentOutput, drive_percent)

    def pod_zeroed(self, pod : int):
        return not self._homes[pod].get()
    ...
    def execute(self):
        ...
        # self.lb_home_state = self.pod_zeroed(2)
        # self.rb_home_state = self.pod_zeroed(3)

        # self.lf_turn_state = self.get_pod_angle(0).degrees()
        # self.rf_turn_state = self.get_pod_angle(1).degrees()
        # self.lb_turn_state = self.get_pod_angle(2).degrees()
        # self.rb_turn_state = self.get_pod_angle(3).degrees()
